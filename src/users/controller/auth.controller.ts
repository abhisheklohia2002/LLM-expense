...
class AuthController {
  ...
  googleAuth = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const refreshToken = this.tokenService.generateRefressToken(
      payload,
      user._id.toString(),
    );
  ...
  refreshToken = async (
    ...
    try {
      ...
      if (isExisted) {
        ...
        res.status(200).json({
          message: "Refresh Token and Access Token Generate successfully",
        });
      }
    } catch (error) {
      const err = createHttpError(500, "user server error");
  ...
  async logout(req: RefreshTokenPayload, res: Response, next: NextFunction) {
    try {
      const auth = req.auth;
      const userId = auth?.sub;
